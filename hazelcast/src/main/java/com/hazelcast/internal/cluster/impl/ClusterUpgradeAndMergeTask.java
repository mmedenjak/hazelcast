 ...
 */
class ClusterUpgradeAndMergeTask implements Runnable {
    ...
    private final Address newTargetAddress;

    ClusterUpgradeAndMergeTask(Config newConfig, Address newTargetAddress, Node node) {
        this.node = node;
    ...
    public void run() {
        ...
        try {
            ...
            node.getStaticConfigEpochService().changeConfig(newConfig);
            node.reinit();
